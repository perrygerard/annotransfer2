...
from remap_annotations import remap_annotations_in_memory

st.set_page_config(page_title="PDF Annotation Remapper", page_icon="📌", layout="centered")


def run_remap(source_bytes, target_bytes, page_num, show_debug):
...
def main():
 ...
 both_uploaded = source_file is not None and target_file is not None

 if not both_uploaded:
 st.info("Upload both PDFs above to enable remapping.")

 clicked = st.button("Remap Annotations", disabled=not both_uploaded, use_container_width=True, type="primary")

 if not clicked:
 return

 st.spinner("Remapping annotations...")

 try:
 source_bytes = source_file.read()
 target_bytes = target_file.read()
 output_bytes, log_lines = run_remap(source_bytes, target_bytes, page_num, show_debug)
 except IndexError:
